...
# KORRIGIERT: -i Flag hinzugefügt, um die "Permission denied"-Warnung zu unterdrücken
sudo -i -u postgres psql -c "ALTER USER ${DB_ADMIN_USER} WITH PASSWORD '${KIVI_DB_PASSWORD}';"
...
# KORRIGIERT: -i Flag hinzugefügt
if sudo -i -u postgres psql -t -c '\du' | cut -d \| -f 1 | grep -qw $DB_USER; then
    print_message "${COLOR_YELLOW}" "${ICON_WARN}" "Benutzer '${DB_USER}' existiert bereits. Ändere Passwort."
    # KORRIGIERT: -i Flag hinzugefügt
    sudo -i -u postgres psql -c "ALTER USER ${DB_USER} WITH PASSWORD '${KIVI_DB_PASSWORD}';"
else
    # KORRIGIERT: -i Flag hinzugefügt
    sudo -i -u postgres psql -c "CREATE USER ${DB_USER} WITH PASSWORD '${KIVI_DB_PASSWORD}';"
...
if sudo -i -u postgres psql -lqt | cut -d \| -f 1 | grep -qw $DB_NAME; then
   ...
   # KORRIGIERT: -i Flag hinzugefügt
   sudo -i -u postgres psql -c "CREATE DATABASE ${DB_NAME} OWNER ${DB_USER} ENCODING 'UTF8' TEMPLATE template0 LC_COLLATE 'de_DE.UTF-8' LC_CTYPE 'de_DE.UTF-8';"
...
# KORRIGIERT: -i Flag hinzugefügt
DB_CHECK_RESULT=$(sudo -i -u postgres psql -t -d ${DB_NAME} -c "SELECT pg_encoding_to_char(encoding) FROM pg_database WHERE datname = '${DB_NAME}';" | xargs)
...
# KORRIGIERT: -i Flag hinzugefügt
PG_HBA_CONF_PATH=$(sudo -i -u postgres psql -t -c "SHOW hba_file;" | xargs)
...
# KORRIGIERT: Trennzeichen von / auf # geändert, um Konflikte mit dem '/' in 127.0.0.1/32 zu vermeiden.
